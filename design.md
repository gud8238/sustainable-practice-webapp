...
## 4. Pattern (v4.1 재구성 — Nodeflow 디자인 에셋 직접 반영)

> v4.1: 사용자가 지정한 Nodeflow 컴포넌트 에셋(`Cards/Stat Card` 노드 LOPjSZ2KX 등)의 **구성 문법 자체**를 반영해
> UI를 재구성함. 페이지별 내용(입력 항목·데이터 필드·기능)은 무변경.

- 플로팅 셸 유지 — 셸은 `--bg` + 1px `--line` 보더, 페이지 바깥은 `--bg-deep` + 상단 중앙 **블루 radial 글로우**.
- **로그인 = Nodeflow 히어로 문법**: 프로젝트에서 추출한 **실제 행성 에셋**(`assets/planet.png`, 투명 PNG)을
  상단 중앙에 배치(셸이 상단을 클리핑해 수평선 효과) + 중앙 정렬 헤드라인 + **인라인 pill 폼**(waitlist 문법:
  pill input + pill CTA 나란히) + 아래 와이드 미디어 카드(hero.mp4).
- **섹션 헤더 = 중앙 정렬**: pill 뱃지 + 큰 헤딩 + 뮤티드 설명 (Nodeflow "How it works?" 문법).
- **활동 카드 = Stat Card 문법**(사용자 지정 에셋): 상단 [아이콘 칩(12px 라운드 사각, 틴트 로테이션) ↔ 그린 틴트
  트렌드 pill(카테고리 태그)] + [큰 숫자 01~04 + 뮤티드 타이틀] + 설명 + 입력.
- **실천흐름 = Step 행 문법**: 세로 리스트, 행마다 헤어라인 상단 보더. 좌측 메타(STEP N pill + 컬러 도트 로테이션,
  큰 헤딩, 기간 뮤티드) / 우측 입력 패널(surface 카드). 4단계(지속)는 블루 30% 보더 강조.
- 매트릭스·요약·푸터는 v4 유지(틴트 로테이션).
